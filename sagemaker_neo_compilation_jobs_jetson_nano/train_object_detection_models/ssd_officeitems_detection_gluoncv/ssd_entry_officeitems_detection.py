...
def parse_args():
    ...
    parser.add_argument('--lr-decay-interval', type=int, default=15,
                        help='epochs at which learning rate decays.')
    parser.add_argument('--momentum', type=float, default=0.9,
                        help='SGD momentum, default is 0.9')
    parser.add_argument('--wd', type=float, default=0.0005,
                        help='Weight decay, default is 5e-4')
    parser.add_argument('--base-dataset', type=str, default='coco',
                        help='dataset used to train base network, default is coco dataset')
...
class GetS3Dataset(gluon.data.Dataset):
    ...
    """
    def __init__(self, label_file = 'labels.lst',s3_label_path='label',s3_data_path = 'data'):
    ...
    def __getitem__(self, idx):
        ...
        # get the id header and label length
        iid, ihdr, ilabel = info[:3]
        del info[:3]
        
        # read image 
        image = mx.image.imread(os.path.join(self.s3_data_path,info[-1].split('/')[-1]))
        ...
        while info:
            ...
            label.append([imW * float(info[1]), imH*float(info[2]), imW*float(info[3]),imH*float(info[4]),int(float(info[0]))])
            del info[:5]
...
if __name__ == '__main__':
    ...
    train_dataset = GetS3Dataset(label_file = 'train.lst', s3_label_path=os.environ["SM_CHANNEL_LABELS"],s3_data_path=os.environ["SM_CHANNEL_TRAIN"])
    val_dataset = GetS3Dataset(label_file= 'val.lst', s3_label_path=os.environ["SM_CHANNEL_LABELS"],s3_data_path=os.environ["SM_CHANNEL_TRAIN"])
